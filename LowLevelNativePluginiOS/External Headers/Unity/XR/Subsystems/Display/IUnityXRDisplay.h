...
/// Flags that represent supported display provider's shared texture layout
typedef enum UnityXRTextureLayoutFlags
{
    /// Shared textures could be configured to a texture2DArray type
    kUnityXRTextureLayoutFlagsTexture2DArray = 1 << 0,

    /// Shared textures could be configured to a texture2D that represents multiple views such as a double-wide layout
    kUnityXRTextureLayoutFlagsSingleTexture2D = 1 << 1,

    /// Shared textures could be configured to multiple texture2D type
    kUnityXRTextureLayoutFlagsSeparateTexture2Ds = 1 << 2,
} UnityXRTextureLayoutFlags;
...
typedef enum UnityXRRenderTextureFlags
{
    ...
    /// This texture can be an unresolved MSAA texture.  Accepting unresolved textures lowers the bandwidth needed by tile-based architectures.
    kUnityXRRenderTextureFlagsSkipResolve = 1 << 1,

    /// Specifies that the resources backing this texture can't be resized.  No control over width / height of texture.
    /// Unity might render to a separate texture of a more convenient size, then blit into this one.
    /// For Example, HoloLens backbuffer size can't be changed.
    kUnityXRRenderTextureFlagsLockedWidthHeight = 1 << 2,

    /// Texture can only be written to and can't be read from. Unity needs to create intermediate textures to do post-processing work.
...
{
    /// @brief A UnityXRRenderPass potentially involves a culling pass and a scene graph traversal.
    ///
    /// This is a resource-intensive operation. You can use single-pass rendering to limit the number of times this operation is performed.
    ///
    /// Each UnityXRRenderPass contains an output texture (which can be a texture array),
    /// and output UnityXRRenderParams such as the rect to render to, or the texture array slice.
    ///
    /// Unity will not perform any rendering operations for the next frame unless at least 1 RenderPass
    /// is declared. Implementations may return 0 Renderpass(es) to indicate that no rendering should
    /// occur, and that Unity should immediately begin simulating the next frame. Alternatively,
    /// implementations may instead choose to block inside of WaitForNextFrameDesc before returning a
    /// RenderPass to indicate Unity should wait until a frame is available.
    ///
    ...
    ///
    /// provider can make these assumptions:
    ...
    struct UnityXRRenderPass
    {
        ...
        /// Parameters for rendering. In the case of single-pass there might be multiple sets of rendering params.
        struct UnityXRRenderParams
        {
            /// Eye pose in device anchor space.  This is usually a static transform that might change for IPD or foveated rendering.
    ...
    {
        /// Culling pose in device anchor space.  This is usually a static transform that might change for IPD or foveated rendering.
        /// Head tracking is applied through the input subsystem by filling out kUnityXRInputFeatureUsageCenterEyePosition.
        /// Pose is in Unity coordinates (forward z is positive).
...
typedef struct UnityXRMirrorViewBlitDesc
{
    ...
    /// If this value is true, Unity might use the registered native bli callback: UnityXRDisplayGraphicsThreadProvider::BlitToMirrorViewRenderTarget
...
{
    /// The provider didn't specify the type of reprojection mode it's using.
    kUnityXRReprojectionModeUnspecified,

    /// Reprojection is based on both the users head position and orientation.
    kUnityXRReprojectionModePositionAndOrientation,

    /// Reprojection is based only on the users head orientation.
    kUnityXRReprojectionModeOrientationOnly,

    /// The engine doesn't use stabilization based on user head information.
...
{
    /// The engine has requested to change underlying shared texture layout.  Check the #UnityXRFrameSetupHints::UnityXRAppSetup `selectedTextureLayoutFlag` field.
    kUnityXRFrameSetupHintsChangedSelectedTextureLayout = 1 << 0,
...
{
    /// Options that the application configures directly. These might change on a per-frame basis.
    struct UnityXRAppSetup
    {
        /// Currently selected shared texture layout flag, could be one of the #UnityXRTextureLayoutFlags.
        UnityXRTextureLayoutFlags selectedTextureLayoutFlag;
        ...
        /// App requests that the provider enables or disables content protection.
        ...
        /// provider might switch back to a default focus plane.
...
{
    /// All supported texture layout should be specified in the flag, could be combination of #UnityXRTextureLayoutFlags.
    uint32_t supportedTextureLayoutFlags;
    ...
    /// Don't present to the main screen (To skip eglSwapBuffers, or other equivalent graphics API present calls).
...
typedef struct UnityXRMirrorViewRenderTargetDescriptor
{
    ...
    /// Mirror view render view render target's texture sample count.
    uint8_t              rtSamples;

    /// Mirror view render target's mips.
    uint8_t              rtMipCount;

    /// Gfx thread native texture, Is NULL on game thread.
...
{
    /// Plugin-specific data pointer. The Unity runtime passes this data
    /// to every function in this structure. The plugin should allocate
    ...
    /// @param[out] renderingCaps Modify if your SDK or the hardware you're using supports specific rendering features, such as texture layout.
    ///
    ...
    /// Submit the previous frame information (for example, eye textures, depth texture) to the compositor.
    /// This call is scheduled just before the native graphics API call to submit / swap buffers for the main window (if applicable).
    /// This call might optionally block for cadence if #PopulateNextFrameDesc doesn't block.
    ///
    /// @param[in] handle Handle obtained from UnityLifecycleProvider callbacks.
    /// @param[in] userData User specified userData that was filled out on #UnityXRDisplayGraphicsThreadProvider
    UnitySubsystemErrorCode(UNITY_INTERFACE_API * SubmitCurrentFrame)(UnitySubsystemHandle handle, void* userData);

    /// PopulateNextFrameDesc will be called after simulation and pending graphics is called.
    ...
    /// @param[in] mirrorBlitInfo Data block that contains render target descriptor and current selected blit mode.
...
typedef struct UnityXRDisplayState
{
    ...
    /// Set this to true if the display is transparent (for example, AR device display).
    ...
    /// Optional legacy native pointer exposed via the XRDevice.GetNativePtr() C# API.
...
{
    /// Pointer which is passed to every callback as the userData parameter.
    void* userData;
    ...
    /// If provider doesn't implement this callback, default blit behavior is used.
    /// If returns 0 blit params, renderer assumes provider doesn't supports mirror view blit.
    /// If returns an error code, renderer assumes provider doesn't supports mirror view blit.
    ///
    /// @param[in] handle Handle obtained from UnityLifecycleProvider callbacks.
    /// @param[in] userData User specified data
    /// @param[in] mirrorBlitInfo Data block that contains render target descriptor and current selected blit mode.
...
{
    /// Entry point for getting callbacks when the display subsystem is initialized / started / stopped / shut down.
    ///
    /// Example usage:
    /// @code
    ...
    /// Create a UnityXRRenderTextureId given a UnityXRRenderTextureDesc. Safe to be called on main thread and gfx thread.
    ///
    /// @param[in] handle Handle obtained from UnityLifecycleProvider callbacks.
    ...
    /// Converts a UnityXRRenderTextureId to a UnityXRRenderTextureDesc.  Useful in order to obtain the platform specific texture information.
    ///
    /// @param[in] handle Handle obtained from UnityLifecycleProvider callbacks.
    ...
    /// Destroy a UnityXRRenderTextureId.  Safe to be called on main thread and gfx thread.
    ///
    /// @param[in] handle Handle obtained from UnityLifecycleProvider callbacks.
...
UNITY_REGISTER_INTERFACE_GUID(0x81568bbe57ba4d7dULL, 0x09876619c6c823201ULL, IUnityXRDisplayInterface)
